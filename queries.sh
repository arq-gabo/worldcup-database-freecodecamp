...
echo "$($PSQL "SELECT ROUND(AVG(winner_goals) + AVG(opponent_goals), 16) AS all_goals FROM games")"
...
echo "$($PSQL "SELECT name FROM teams WHERE team_id = ANY (SELECT winner_id FROM games WHERE round = 'Eighth-Final' AND year = 2014 UNION ALL SELECT opponent_id FROM games WHERE round = 'Eighth-Final' AND year = 2014) ORDER BY name")"
